...
function InfoBlock({ title, text, isInstances, screenshotPath }) {
  const isScreenshot = title === "Screenshots";
  
  return (
    <div style={styles.infoBlock}>
      <span style={styles.blockLabel}>{title}</span>
      {isScreenshot && screenshotPath ? (
        <div style={{
          marginTop: '8px',
          ...
          <img 
            ...
            onError={(e) => {
              ...
              e.target.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="100"><text x="50%" y="50%" text-anchor="middle" fill="%23999">No Image</text></svg>';
            }}
          />
        </div>
      ) : isInstances ? (
