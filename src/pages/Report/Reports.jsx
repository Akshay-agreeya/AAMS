...
import { getFormattedDateWithTime } from "../../component/input/DatePicker"; // ✅ Import date formatter
import './reports.css'
...
const Reports = () => {
  ...
  // ✅ UPDATED: Store array of Excel reports instead of single report
  const [excelReports, setExcelReports] = useState([]);
  const [loadingExcelReports, setLoadingExcelReports] = useState(false);
  ...
  useEffect(() => {
    if (selectedOrg) {
      getReports();
      getExcelReportsForOrg(); // ✅ NEW: Fetch Excel reports for selected org
    }
  }, [selectedOrg]);
  ...
  // ✅ NEW: Fetch all Excel reports for the selected organization
  const getExcelReportsForOrg = async () => {
    try {
      setLoadingExcelReports(true);
      const resp = await getData(`/accessibility/org/${selectedOrg}/assessments`);
      setExcelReports(resp?.data || []);
    } catch (err) {
      console.error('Error fetching Excel reports:', err);
      setExcelReports([]);
    } finally {
      setLoadingExcelReports(false);
    }
  };
                      ...
                      <div className="reportListingGridContainer">
                        {loading ? (
                          ...
                          <>
                            ...
                            {/* ✅ NEW: Excel Reports - Loop through all assessments for this org */}
                            {loadingExcelReports ? (
                              <div className="reportListingRepeat">
                                <Loading />
                              </div>
                            ) : (
                              excelReports.map((report, index) => (
                                <div className="reportListingRepeat excel-report-card" key={`excel-${index}`}>
                                  <div className="box">
                                    <div className="siteIcon">
                                      <img src={blackSiteIcon} alt="Excel Report" />
                                    </div>
                                    <div className="siteName">
                                      {report.prepared_for || "Unnamed Report"}
                                    </div>
                                  </div>
                                  <div className="box">
                                    <div className="accessbilityDescription">
                                      ...
                                      <div className="desc">
                                        ...
                                        {report?.assessment_timestamp
                                          ? getFormattedDateWithTime(
                                              new Date(report.assessment_timestamp),
                                              "dd MMM yyyy HH:mm"
                                            )
                                          : "—"}
                                      </div>
                                  ...
                                  <div className="navigateICon">
                                    <button
                                      onClick={() =>
                                        // navigate(`/reports/excel/${report.assessment_id}`)
                                        navigate(`/accessibility-report?assessmentId=${report.assessment_id}`)
