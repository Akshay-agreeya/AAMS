...
const location = useLocation();
    ...
    useEffect(() => {
        if (manualReportData && selectedPageUrl) {
            formRef.current.setFieldValue("page_url", selectedPageUrl);
            const data = manualReportData.find(item => item.pageUrl === selectedPageUrl);
            const conData = [];
            data?.formData?.forEach(item => {
                item.conditions.forEach(cItem => {
                    conData.push({ condition_id: cItem.condition_id, status_id: cItem.status_id });
                })
            })
            setFormDataManual(conData);
        }
...
export default EditManualAssessment;
